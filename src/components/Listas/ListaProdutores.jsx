...
const columns = [
  ...
  { field: "nome", headerName: "Nome", width: 230, align: "left" },
  { field: "cidade", headerName: "Cidade", width: 130, align: "left" },
  {
    field: "contato",
    headerName: "Contato",
    width: 150,
...
export default function ListaProdutores({
  ...
  const excluir = async () => {
    ...
    await deleteDoc(doc(db, "produtores", idExclusao.id));
    getProdutores();
    handleClose();
  };
  ...
  return (
    <Paper sx={{ width: "100%", overflow: "hidden" }}>
      <TableContainer>
        <Table size="small" stickyHeader aria-label="sticky table dense">
          <TableHead>
            <TableRow>
              {columns.map((column) => (
      ...
      >
        <DialogTitle id="responsive-dialog-title" style>
          Deseja excluir produtor: {idExclusao.nome} ?
        </DialogTitle>
        <DialogActions>
